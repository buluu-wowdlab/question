...
export default function NameEntry({ name, onNameChange }: NameEntryProps) {
  ...
  if (!editing && name) {
    return (
      <div className="mb-4 flex items-center justify-between text-sm text-gray-700 lg:mb-6 lg:text-lg">
        <span>{name} 님</span>
        <button
          type="button"
          onClick={() => {
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-4 flex gap-2 lg:mb-6">
      <input
        value={input}
        onChange={(e) => setInput(e.target.value)}
        maxLength={20}
        placeholder="이름을 입력하세요"
        className="flex-1 rounded-lg border border-gray-300 px-3 py-2 text-sm lg:px-4 lg:py-3 lg:text-lg"
      />
      <button
        type="submit"
