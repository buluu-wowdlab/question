...
export default function QuestionForm({ disabled, onSubmit }: QuestionFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-6 flex flex-col gap-2 md:mb-7 md:gap-2.5 lg:mb-8 lg:gap-3">
      <textarea
        value={text}
        ...
        className="w-full rounded-lg border border-gray-300 p-3 text-sm disabled:bg-gray-100 md:p-3.5 md:text-base lg:p-4 lg:text-lg"
      />
      <div className="flex items-center justify-between">
        <span className="text-xs text-gray-400 md:text-sm">{text.length}/500</span>
        <button
          type="submit"
      ...
      {error && <p className="text-sm text-red-500 md:text-base lg:text-base">{error}</p>}
    </form>
  )
}
